...
export const removeUI = () => {
  if (elements.firstSection.parentNode) {
    elements.firstSection.parentNode.removeChild(elements.firstSection);
  }
};
...
export const renderResultUI = (el, url, foods) => {
  ...
  if (el.length === 0) {
    markup =
      "<li>با این چیزایی که خونه داری باید تلفن را برداری زنگ بزنی رستوران</li>";
    elements.results.insertAdjacentHTML("beforeend", markup);
  } else {
    el.forEach((element) => {
      markup = `
                <li><a href=${
                  url[Object.keys(foods).indexOf(element)]
                }>${element.split("_").join(" ")}</a></li>
            `;
      elements.results.insertAdjacentHTML("beforeend", markup);
    });
  }
